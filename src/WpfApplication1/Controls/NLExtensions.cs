...
namespace AssetBuilder.Controls
{
	static class NLExtensions
	{
		...
		static NLExtensions()
		{
            ...
            valid.Add('{', @"\{((qv|qs|qd)\d+(\|\d+){0,1}|(qv|qs|qd|qt|nt)\d+|(ca|ga|qa|ce)[-]{0,1}\d+|((qp(\d+\|)([-]{0,1}\d+\|)|(cc|gc|qc|qn)(\d+\|){1})(?s:([^\|\}]*\|){0,1})|(lb|ln|ll|lr|la|lo)(?s:[^\}]*)|(xf|xm|xx|xy|xz)([^\|\}]*(\|[^\|\}]*){0,1})|\d{3}-\d{3}(?i:[ymwd]))(?s:[^\|\}]*))\}");
			valid.Add('[', @"\[(?![A-Z][^/]*/[a-z])(?![a-z][^/]*/[A-HJ-Z])[^/]*/((?!\b(?i:he)\b)(?!\bi\b)[^/])*\]");
			valid.Add('~', @"~((T|E|I)[^~#]*#|W\d{2}|M|F|P)[^~]*~");
			valid.Add('<', @"(<\w+\b([ ]+\w[\w-]+=""[^""<>]*"")*[ ]*(>|/>)|</\w+>)");
			NLErrors.Add('{', new List<NLError>());
			NLErrors.Add('[', new List<NLError>());
            NLErrors['{'].Add(new NLError { Key = @"\{(qc|qn|qv|qs|qd|ca|ga|qa|ce|qt|qp|cc|gc|nt|lb|ln|ll|lr|la|lo|xf|xm|xx|xy|xz)[^\}]*\}", Value = "Missing clause descriptor. (i.e. 'qp', 'qa', etc.)" });
            NLErrors['{'].Add(new NLError { Key = @"\{((qv|qs|qd|ca|ga|qa|ce|qt|nt)\d+|(qc|qn|qp|cc|gc)\d+\||(lb|ln|ll|lr|la|lo|xf|xm|xx|xy|xz))[^\}]*\}", Value = "Missing id and/or pipe after clause descriptor. (i.e. 'cc1234|...')" });
            NLErrors['{'].Add(new NLError { Key = @"\{(qp(\d+\|)([-]{0,1}\d+\|)|(qc|qn|qv|qs|qd|ca|ga|qa|ce|qt|nt|lb|ln|ll|lr|la|lo|xf|xm|xx|xy|xz))[^\}]*\}", Value = "Missing answerid and/or pipe after questionid. (i.e. 'qp1234|2|...')" });
            NLErrors['{'].Add(new NLError { Key = @"\{(qv|qs|qd)\d+(\|\d+){0,1}|(qp(\d+\|)([-]{0,1}\d+\|)([^\|]*\|){0,1}|(cc|gc|qc|qn)(\d+\|){1}([^\|]*\|){0,1}|(ca|ga|qa|ce|qt|nt)\d+|(lb|ln|ll|lr|la|lo)([^\|]*\|)*)[^\}\|]*\}|(xf|xm|xx|xy|xz)([^\|\}]*(\|[^\|\}]*){0,1})", Value = "Too many pipes. (i.e. 'qp1234|2|...|...|')" });
